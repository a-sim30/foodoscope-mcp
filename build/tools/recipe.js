...
export function registerRecipeTools(server) {
    server.registerTool("get_recipe_info", {
        ...
        const data = await get("RecipeDB3", "/recipesinfo", { id: recipe_id });
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
    server.registerTool("recipes_by_cuisine", {
        title: "Recipes By Cuisine",
        description: "Fetch recipes filtered by world cuisine/region from RecipeDB.",
        inputSchema: cuisineSchema.shape,
    }, async ({ region, limit }) => {
        const data = await get("RecipeDB3", `/recipes_cuisine/cuisine/${region}`, { limit });
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
    server.registerTool("recipe_of_the_day", {
        title: "Recipe Of The Day",
        description: "Get the daily featured recipe from RecipeDB.",
        inputSchema: {},
    }, async () => {
        const data = await get("RecipeDB3", "/recipeofday");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
}
